...
namespace Mane.UI.Editor
{
    ...
    public class MaxTMProSizeEditor : UnityEditor.Editor
    {
        ...
        {
            _textProperty = serializedObject.FindProperty(MaxTMProSize.TextPropertyName);
            _layoutElementProperty = serializedObject.FindProperty(MaxTMProSize.LayoutElementPropertyName);
            _contentSizeFitterProperty = serializedObject.FindProperty(MaxTMProSize.ContentSizeFitterPropertyName);
            
            _maxWidthProperty = serializedObject.FindProperty(MaxTMProSize.MaxWidthPropertyName);
            _maxHeightProperty = serializedObject.FindProperty(MaxTMProSize.MaxHeightPropertyName);
        ...
        private void LayoutElementField(SerializedProperty property, Func<RectTransform, float> defaultValue)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                property.floatValue = (enabled ? defaultValue((target as MaxTMProSize).transform as RectTransform) : -1);
            }
